      ...
      <div className="relative transition-all duration-300 ease-in-out">
        <div className={`absolute inset-0 transition-all duration-300 ease-in-out ${
          scrolled
            ...
            : "bg-gradient-to-r from-[#f54a00]/70 via-orange-500/60 to-amber-400/70 backdrop-blur-lg border-white/20 shadow-lg"
        }`} />
            ...
            <nav className="hidden md:flex items-center gap-6">
              ...
              <button
                ...
                className={`text-sm font-medium transition-colors ${
                  menuActive
                    ? "text-[#f54a00] font-semibold border-b-2 border-[#f54a00] pb-0.5"
                    : "hover:text-[#f54a00]"
                }`}>
                Menu
              </button>
        ...
        <div className="md:hidden bg-gradient-to-b from-[#f54a00] via-orange-600 to-amber-400 text-white">
          <div className="px-4 py-4 space-y-3">
            <Link href="/" onClick={() => setOpen(false)}
              className={`block ${linkClass("/")}`}>Home</Link>

            <button onClick={() => { scrollToSection("menu"); setOpen(false); }}
              className={`block text-left text-sm font-medium ${menuActive ? "text-white font-semibold underline" : ""}`}>
              Menu
            </button>

            <Link href="/catering" onClick={() => setOpen(false)}
              className={`block ${linkClass("/catering")}`}>Catering</Link>
            <Link href="/about" onClick={() => setOpen(false)}
              className={`block ${linkClass("/about")}`}>About</Link>
            <Link href="/contact" onClick={() => setOpen(false)}
              className={`block ${linkClass("/contact")}`}>Contact</Link>
